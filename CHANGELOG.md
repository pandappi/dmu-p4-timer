...
- **설정 확장(`SettingsModal`)** — 언어, TTS 음량, 파티콜 복사 옵션
  (파티챗 / 사운드) 추가.
- **알림 TTS 우선도·큐잉** — 같은 시점에 여러 알림이 겹칠 때 가속도폭탄을
  맨 뒤로 미루고(낮은 우선도), 앞 음성을 끊지 않고 이어서 재생하도록 큐잉
  (`speak`의 `enqueue` 옵션 + 알림 effect 배치 처리). 물/번개 콜이 끝난 뒤
  폭탄 콜이 연이어 나온다.
- **2차 진실/거짓 미리 선택** — 설정에서 옵션을 켜면 1차 입력 화면에서
  2차 진실/거짓을 선택해둘 수 있고, 2차 입력 화면에 자동 반영된다.
- **가속도폭탄 공통 파티콜** — 파티모드에서 1차/3차 진실·거짓이 같으면
  파티콜 요약 앞에 "가속도 모두 멈추기/움직이기"를 강조 표시하고 복사
  문자열에도 포함한다.
- **물/번개 처리법 색상 강조** — 파티콜 요약과 처리 순서에서 물 뒤 처리법은
  하늘색, 번개 뒤 처리법은 노란색으로 강조한다. 라이트/다크 테마별 대비
  색상을 별도로 적용한다.
...
- **처리법(행동) 텍스트** — 디버프 + 진실/거짓 조합으로 만료 시 행동을 표시
  ...
  멈추기/움직이기, 저주의 외침 뒤돌기/마안보기, Entropy·Dynamic Fluid는
  원형장판피하기/도넛장판.
